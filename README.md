...
- **Configurable max_tokens** — the per-call output budget resolves in priority order: `ACTIVEWIKI_CHECK_MAX_TOKENS` env var > `quality_check.max_tokens` in `activewiki.json` > script default 4096, with a hard cap of 32768 (values above the cap are clamped with a warning — protects the model context window). Garbage values (non-numeric, 0, negative) are ignored and the next priority level applies; a missing config key never fails fast. Increase the value when reports show `abgeschnitten (max_tokens zu klein?)`.
- **Hardened answer parser** — `parse_llm_answer()` returns `(parsed, raw, grund)` and distinguishes failure causes: balanced-brace scanning (string escapes respected, not blind first-`{`-to-last-`}`), Markdown code-fence stripping at the edges, and a distinct reason `abgeschnitten (max_tokens zu klein?)` for truncated JSON vs `kein JSON` for prose. The error issue in the report names the reason and, on truncation, suggests raising `quality_check.max_tokens`.
