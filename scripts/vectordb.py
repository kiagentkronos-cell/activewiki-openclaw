...
# ── HTML Graph Visualization (Phase 2C) ────────────────────────────
# Hard limits for the force-directed graph to keep browser responsive
HTML_GRAPH_MAX_NODES = 200
HTML_GRAPH_MAX_EDGES = 500

# Entity-type → CSS color mapping for node coloring
ENTITY_TYPE_COLORS: Dict[str, str] = {
    "PERSON":       "#4A90D9",   # blue
    "ORGANIZATION": "#7B68EE",   # slate blue
    "LOCATION":     "#2ECC71",   # green
    "DOCUMENT":     "#E67E22",   # orange
    "PROPERTY":     "#E74C3C",   # red
    "FACILITY":     "#F39C12",   # amber
    "CONCEPT":      "#95A5A6",   # grey
    "DATE":         "#1ABC9C",   # teal
    "MONEY":        "#27AE60",   # emerald
    "EVENT":        "#E91E63",   # pink
    "RATIONALE":    "#9B59B6",   # purple
}
DEFAULT_NODE_COLOR = "#BDC3C7"   # light grey fallback

# Confidence → link width mapping (extracted=thick, ambiguous=thin)
CONFIDENCE_LINK_WIDTH: Dict[str, float] = {
    "extracted": 3.0,
    "inferred":  2.0,
    "ambiguous": 1.0,
}
DEFAULT_LINK_WIDTH = 2.0

# Path to the local D3.js minified file (no CDN — privacy requirement)
_D3_LOCAL_PATH = Path(__file__).resolve().parent.parent / "assets" / "d3.v7.min.js"
...
# ── D3.js HTML Template ─────────────────────────────────────────────
# Inline D3.js from local file (no CDN — privacy requirement).
# Placeholders: {NODES_JSON}, {LINKS_JSON}, {D3_SCRIPT}, {TITLE}, {BANNER}
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; overflow: hidden; background: #0d1117; color: #c9d1d9; }

  /* Confidential banner */
  #banner { position: fixed; top: 0; left: 0; right: 0; height: 28px; background: #da3633; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 12px; font-weight: 700; letter-spacing: 0.5px; z-index: 1000; }

  /* Header bar */
  #header { position: fixed; top: 28px; left: 0; right: 0; height: 44px; background: #161b22; border-bottom: 1px solid #30363d; display: flex; align-items: center; padding: 0 16px; gap: 12px; z-index: 999; }
  #header h1 { font-size: 14px; font-weight: 600; color: #58a6ff; }
  #header .stats { font-size: 12px; color: #8b949e; }
  #header input[type="text"] { background: #0d1117; border: 1px solid #30363d; color: #c9d1d9; padding: 4px 10px; border-radius: 6px; font-size: 13px; width: 220px; outline: none; }
  #header input[type="text"]:focus { border-color: #58a6ff; }
  #header .filter-group { display: flex; gap: 6px; margin-left: auto; align-items: center; }
  #header .filter-group label { font-size: 11px; color: #8b949e; cursor: pointer; display: flex; align-items: center; gap: 3px; }
  #header .filter-group input[type="checkbox"] { accent-color: #58a6ff; }

  /* Main SVG area */
  #graph-svg { display: block; margin-top: 72px; }

  /* Sidebar panel */
  #sidebar { position: fixed; top: 72px; right: 0; bottom: 0; width: 340px; background: #161b22; border-left: 1px solid #30363d; transform: translateX(100%); transition: transform 0.2s ease; z-index: 998; overflow-y: auto; padding: 16px; }
  #sidebar.open { transform: translateX(0); }
  #sidebar h2 { font-size: 16px; color: #58a6ff; margin-bottom: 4px; }
  #sidebar .entity-type { display: inline-block; font-size: 11px; padding: 2px 8px; border-radius: 12px; margin-bottom: 8px; color: #fff; }
  #sidebar .section-title { font-size: 12px; font-weight: 600; color: #8b949e; margin-top: 14px; margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.5px; }
  #sidebar .rel-item { font-size: 12px; padding: 4px 0; border-bottom: 1px solid #21262d; }
  #sidebar .rel-item .rel-type { color: #ffa657; font-weight: 600; }
  #sidebar .rel-item .rel-target { color: #c9d1d9; }
  #sidebar .rel-item .rel-conf { font-size: 10px; padding: 1px 5px; border-radius: 4px; margin-left: 4px; }
  #sidebar .rel-conf.extracted { background: #238636; color: #fff; }
  #sidebar .rel-conf.inferred { background: #1f6feb; color: #fff; }
  #sidebar .rel-conf.ambiguous { background: #6e7681; color: #fff; }
  #sidebar .close-btn { position: absolute; top: 12px; right: 12px; background: none; border: none; color: #8b949e; font-size: 18px; cursor: pointer; }
  #sidebar .close-btn:hover { color: #f85149; }

  /* Legend */
  #legend { position: fixed; bottom: 16px; left: 16px; background: rgba(22,27,34,0.9); border: 1px solid #30363d; border-radius: 8px; padding: 12px; font-size: 11px; z-index: 997; }
  ...
  #legend .legend-line.thick { border-top-width: 3px; }
  #legend .legend-line.thin { border-top-width: 1px; border-style: dotted; }

  /* Tooltip */
  #tooltip { position: fixed; background: #1c2128; border: 1px solid #30363d; border-radius: 6px; padding: 6px 10px; font-size: 12px; pointer-events: none; opacity: 0; transition: opacity 0.15s; z-index: 1001; max-width: 250px; }
</style>
</head>
<body>
...
<div id="header">
  ...
  <input type="text" id="search-input" placeholder="Suche nach Knoten…" />
  <div class="filter-group">
    <label><input type="checkbox" id="filter-extracted" checked /> extracted</label>
    <label><input type="checkbox" id="filter-inferred" checked /> inferred</label>
    <label><input type="checkbox" id="filter-ambiguous" checked /> ambiguous</label>
  </div>
</div>
...
<div id="legend">
  ...
  <div class="legend-row"><span class="legend-line thick" style="border-color:#58a6ff;"></span> extracted</div>
  <div class="legend-row"><span class="legend-line" style="border-color:#58a6ff;"></span> inferred</div>
  <div class="legend-row"><span class="legend-line thin" style="border-color:#6e7681;"></span> ambiguous</div>
</div>
...
(function() {
  ...
  // ── Color helpers ─────────────────────────────────────────────
  const typeColor = d => d.color || '#BDC3C7';
  const confWidth = d => d.width || 2;
  const confDash  = d => d.confidence === 'ambiguous' ? '4,3' : 'none';
  const confOpacity = d => d.confidence === 'ambiguous' ? 0.4 : (d.confidence === 'inferred' ? 0.7 : 1);
  ...
  const link = linkGroup.selectAll('line')
    ...
    .join('line')
    .attr('stroke', '#58a6ff')
    .attr('stroke-opacity', d => confOpacity(d))
    .attr('stroke-width', d => confWidth(d))
    .attr('stroke-dasharray', d => confDash(d));
  ...
  node.on('mouseover', function(event, d) {
    d3.select(this).transition().duration(100).attr('r', 12).attr('stroke', '#58a6ff');
    label.filter(l => l.id === d.id).style('opacity', 1);
    tooltip.style('opacity', 1)
      .html(`<strong>${d.label}</strong><br/><span style="color:#8b949e">${d.type}</span>`)
      .style('left', (event.pageX + 12) + 'px')
      .style('top', (event.pageY - 10) + 'px');
  })
  .on('mouseout', function(event, d) {
    if (!selectedNode) {
      d3.select(this).transition().duration(100).attr('r', 8).attr('stroke', '#0d1117');
  ...
  .on('click', function(event, d) {
    ...
    selectedNode = d;
    d3.select(this).transition().duration(100).attr('r', 14).attr('stroke', '#ffa657');
    label.filter(l => l.id === d.id).style('opacity', 1);
    showSidebar(d);
  });
  ...
  svg.on('click', () => {
    if (selectedNode) {
      selectedNode = null;
      node.transition().duration(200).attr('r', 8).attr('stroke', '#0d1117');
      label.style('opacity', 0);
      d3.select('#sidebar').classList.remove('open');
    }
  });
  ...
  function showSidebar(d) {
    ...
    const outRel = links.filter(l => l.source.id === d.id);
    const inRel  = links.filter(l => l.target.id === d.id);

    let html = `
      <h2>${d.label}</h2>
      <span class="entity-type" style="background:${typeColor(d)}">${d.type}</span>
      ${d.description ? '<p style="font-size:12px;color:#8b949e;margin-top:6px;">' + d.description + '</p>' : ''}
      ${d.wiki_page ? '<p style="font-size:11px;color:#8b949e;margin-top:4px;">📄 ' + d.wiki_page + '</p>' : ''}
    `;

    if (outRel.length) {
      html += '<div class="section-title">→ Outgoing</div>';
      outRel.forEach(r => {
        const t = typeof r.source === 'object' ? r.source : nodes.find(n => n.id === r.source);
        html += `<div class="rel-item"><span class="rel-type">${r.type}</span> → <span class="rel-target">${t.label}</span><span class="rel-conf ${r.confidence}">${r.confidence}</span></div>`;
      });
    }
    if (inRel.length) {
      html += '<div class="section-title">← Incoming</div>';
      inRel.forEach(r => {
        const t = typeof r.target === 'object' ? r.target : nodes.find(n => n.id === r.target);
        html += `<div class="rel-item"><span class="rel-target">${t.label}</span> <span class="rel-type">${r.type}</span><span class="rel-conf ${r.confidence}">${r.confidence}</span></div>`;
      });
    }
  ...
  d3.select('#search-input').on('input', function() {
    ...
    node.style('opacity', d => matchIds.has(d.id) ? 1 : 0.15)
      .attr('r', d => matchIds.has(d.id) ? 12 : 6);
    link.style('opacity', d => matchIds.has(d.source.id) && matchIds.has(d.target.id) ? 1 : 0.05);
    label.style('opacity', d => matchIds.has(d.id) ? 1 : 0);
  });

  // ── Confidence filters ────────────────────────────────────────
  ['extracted','inferred','ambiguous'].forEach(conf => {
    d3.select('#filter-' + conf).on('change', function() {
      const checked = this.checked;
      link.filter(d => d.confidence === conf)
        .transition().duration(200)
        .style('opacity', checked ? d => confOpacity(d) : 0);
    });
  });

  // ── Legend population ─────────────────────────────────────────
  const typeSet = new Map();
  nodes.forEach(n => { if (!typeSet.has(n.type)) typeSet.set(n.type, n.color); });
  const legendTypes = d3.select('#legend-types');
  typeSet.forEach((color, type) => {
    legendTypes.append('div').attr('class','legend-row')
      .html(`<span class="legend-dot" style="background:${color}"></span>${type}`);
  });
...
def _read_d3_script() -> str:
    """Read the local D3.js minified file. Raises SystemExit if missing."""
    if not _D3_LOCAL_PATH.exists():
        raise SystemExit(
            f"D3.js local file not found at {_D3_LOCAL_PATH}. "
            f"Download it: curl -o {_D3_LOCAL_PATH} https://cdn.jsdelivr.net/npm/d3@7/dist/d3.min.js"
        )
...
def _build_graph_data(
    ...
    """Extract entities + relationships for the HTML graph visualization.

    Strategy: search entities by label/description LIKE query, then collect
    all 1-hop neighbours (both directions) and their inter-relationships.

    Args:
        conn: Open SQLite connection.
        query: Text to match against entity labels/descriptions.
        min_confidence: If set, only include relationships with confidence
            >= this level (uses CONFIDENCE_ORDER numeric mapping).
        scopes: Optional scope prefix filter on wiki_page.

    Returns:
        (nodes, links) ready for JSON serialization into the D3 template.
    """
    like = f"%{query}%"

    # Step 1: Seed entities matching the query
    ...
    # Step 2: Collect 1-hop neighbour IDs (both directions)
    placeholders = ",".join("?" * len(seed_ids))
    neighbour_ids = set()

    # Outgoing: seed → neighbour
    for row in conn.execute(
        f"SELECT DISTINCT target_id FROM relationships WHERE source_id IN ({placeholders}) AND valid_until IS NULL",
        seed_ids,
    ).fetchall():
        neighbour_ids.add(row[0])

    # Incoming: neighbour → seed
    for row in conn.execute(
        f"SELECT DISTINCT source_id FROM relationships WHERE target_id IN ({placeholders}) AND valid_until IS NULL",
        seed_ids,
    ).fetchall():
        neighbour_ids.add(row[0])

    all_node_ids = set(seed_ids) | neighbour_ids

    # Step 3: Fetch all node details
    node_ph = ",".join("?" * len(all_node_ids))
    all_nodes = conn.execute(
        f"SELECT id, label, entity_type, description, wiki_page FROM entities WHERE id IN ({node_ph})",
        list(all_node_ids),
    ).fetchall()

    # Step 4: Fetch all relationships between these nodes
    all_rels = conn.execute(
        f"SELECT id, source_id, target_id, relation_type, description, confidence "
        f"FROM relationships WHERE source_id IN ({node_ph}) AND target_id IN ({node_ph}) AND valid_until IS NULL",
        list(all_node_ids) + list(all_node_ids),
    ).fetchall()

    # Apply confidence filter if specified
    if min_confidence:
        min_order = CONFIDENCE_ORDER.get(min_confidence, 0)
        all_rels = [
            r for r in all_rels
            if CONFIDENCE_ORDER.get(r[5], 0) >= min_order
        ]

    # Step 5: Build nodes list for D3
    nodes = []
    for nid, nlabel, ntype, ndesc, npage in all_nodes:
        nodes.append({{
            "id": nid,
            "label": nlabel,
            "type": ntype,
            "color": ENTITY_TYPE_COLORS.get(ntype, DEFAULT_NODE_COLOR),
            "description": (ndesc or "")[:120],
            "wiki_page": npage or "",
        }})

    # Step 6: Build links list for D3
    links = []
    for rid, src, tgt, rtype, rdesc, rconf in all_rels:
        links.append({{
            "source": src,
            "target": tgt,
            "type": rtype,
            "confidence": rconf or DEFAULT_CONFIDENCE,
            "width": CONFIDENCE_LINK_WIDTH.get(rconf or DEFAULT_CONFIDENCE, DEFAULT_LINK_WIDTH),
            "description": (rdesc or "")[:120],
        }})
...
def _generate_html_graph(
    ...
    """Generate a static HTML file with D3.js force-directed graph.

    Args:
        nodes: List of node dicts (id, label, type, color, description, wiki_page).
        links: List of link dicts (source, target, type, confidence, width, description).
        query: The search query used (for the title).
        output_dir: Directory for the output file. Defaults to VECTORDB.

    Returns:
        The absolute path to the generated HTML file.

    Raises:
        SystemExit: If node/edge counts exceed hard limits, or D3.js file is missing.
    """
    # ── Hard limits ─────────────────────────────────────────────
    if len(nodes) > HTML_GRAPH_MAX_NODES:
        raise SystemExit(
            f"Graph too large: {len(nodes)} nodes (max {HTML_GRAPH_MAX_NODES}). "
            f"Use a more specific query or reduce scope."
        )
    if len(links) > HTML_GRAPH_MAX_EDGES:
        raise SystemExit(
            f"Graph too large: {len(links)} edges (max {HTML_GRAPH_MAX_EDGES}). "
            f"Use a more specific query or reduce scope."
        )

    # ── Prepare output directory ────────────────────────────────
    out_dir = Path(output_dir) if output_dir else VECTORDB
    out_dir.mkdir(parents=True, exist_ok=True)

    # ── Filename with query hash for uniqueness ─────────────────
    query_hash = hashlib.md5(query.encode()).hexdigest()[:8]
    filename = f"graph_{query_hash}.html"
    out_path = out_dir / filename

    # ── Read D3.js from local file ──────────────────────────────
    ...
    # ── Render template ─────────────────────────────────────────
    has_private = any(n.get("wiki_page", "").startswith("private/") for n in nodes)
    banner = '<div id="banner">⚠ CONFIDENTIAL — Contains private data</div>' if has_private else ""

    html = _D3_HTML_TEMPLATE \
        .replace('{{TITLE}}', f"Knowledge Graph — {query}") \
        .replace('{{NODES_JSON}}', nodes_json) \
        .replace('{{LINKS_JSON}}', links_json) \
        .replace('{{D3_SCRIPT}}', d3_script) \
...
def cmd_graph_html(
    ...
    Queries entities matching *query*, collects 1-hop neighbours, and renders
    a D3.js force-directed graph as a self-contained HTML file.

    Args:
        query: Text to match against entity labels and descriptions.
        min_confidence: Minimum confidence level (extracted/inferred/ambiguous).
        scopes: Optional scope prefix filter.
        output_dir: Output directory (defaults to VECTORDB).
    """
    if not DB_PATH.exists():
        log("error", " Database not found. Run `vectordb.py graph build` first.", stderr=True)
        sys.exit(1)
    ...
    try:
        nodes, links = _build_graph_data(conn, query, min_confidence=min_confidence, scopes=scopes)
    finally:
        conn.close()

    if not nodes:
        log("info", f"No entities matching '{query}' — no graph generated.")
        return

    out_path = _generate_html_graph(nodes, links, query, output_dir=output_dir)
    log("info", f"Graph HTML generated: {out_path} ({len(nodes)} nodes, {len(links)} edges)")
    log("info", f"Open in browser: file://{out_path}")
...
def main() -> int:
    ...
    # graph html (Phase 2C: HTML visualization)
    sp_gh = graph_sub.add_parser("html", help="Generate interactive HTML graph visualization")
    sp_gh.add_argument("query", help="Search query to find seed entities")
    sp_gh.add_argument("--min-confidence", choices=["extracted", "inferred", "ambiguous"],
                        default=None, help="Minimum confidence level for edges")
    sp_gh.add_argument("--scopes", type=str, default=None,
                       help="Comma-separated list of scopes to filter by")
    sp_gh.add_argument("--output-dir", type=str, default=None,
                       help="Output directory (default: vectordb/)")
