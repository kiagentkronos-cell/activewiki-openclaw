...
function resolveWikiRoot() {
    ...
    throw new Error("Cannot resolve wiki root. Set ACTIVEWIKI_WIKIS_ROOT or ACTIVEWIKI_CONFIG.");
}
// ── Constants ────────────────────────────────────────────────────────────────
 ...
 */
function safeResolve(scope, subdir, slug) {
 ...
 */
export function getWikiPage(scope, slug, fromLine, lineCount) {
    // Validate slug — reject anything that could be path traversal
    if (!SLUG_REGEX.test(slug)) {
        console.error(`[activewiki] Invalid slug rejected: "${slug}"`);
        return null;
    }
