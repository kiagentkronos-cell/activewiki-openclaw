...
function resolveWikisRoot() {
    ...
    throw new Error("Cannot resolve wikis_root. Set ACTIVEWIKI_WIKIS_ROOT or ACTIVEWIKI_CONFIG.");
}
// ── Derived constants ────────────────────────────────────────────────────────
...
function buildSafeEnv() {
    ...
    if (!env.HOME)
        env.HOME = process.env.HOME ?? (process.platform === 'win32' ? process.env.USERPROFILE || '' : '');
    if (!env.LANG)
 ...
 */
export async function searchWiki(query, scopes, maxResults) {
    // 1. Over-fetch vector hits so the bridge sees enough distinct pages.
    const vectorPool = await vectorSearch(query, scopes, Math.min(Math.max(maxResults * 3, 12), 30));
    ...
    for (const h of adjusted) {
        ...
        if (!pages.includes(page))
            pages.push(page);
        if (pages.length >= BRIDGE_MAX_PAGES)
            break;
    }
...
function buildKgSnippet(entity) {
...
async function vectorSearch(query, scopes, maxResults) {
    ...
    let stdout;
    try {
        const { stdout: raw } = await execFileAsync(PYTHON_BIN, [VECTORDB_SCRIPT, ...args], {
        ...
        console.error(`[activewiki] vector search failed: ${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
    let hits;
    try {
        ...
        if (!Array.isArray(parsed)) {
            console.error(`[activewiki] unexpected JSON shape from vectordb.py (expected array)`);
            return [];
        }
        hits = parsed;
    }
    catch (err) {
        console.error(`[activewiki] JSON parse error: ${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
    return hits.map((hit) => ({
        id: hit.id,
 ...
 */
async function graphSearchByPages(pages, scopes) {
    if (pages.length === 0)
        return [];
    const scopesArg = scopes.filter(Boolean).join(",");
    const args = [
    ...
    try {
        ...
        console.warn(`[activewiki] graph pages failed (non-blocking): ${err instanceof Error ? err.message : String(err)}`);
    ...
    try {
        ...
        console.warn(`[activewiki] graph JSON parse failed: ${err instanceof Error ? err.message : String(err)}`);
        return [];
    }
    return entities;
}
