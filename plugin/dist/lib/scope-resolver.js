 ...
 */
function resolveScopesConfigPath() {
    ...
    throw new Error("Cannot resolve scopes config. Set ACTIVEWIKI_CONFIG or ACTIVEWIKI_SCOPES_CONFIG.");
}
...
function loadScopesConfig() {
    ...
    try {
        ...
        console.error(`[activewiki] Failed to load scopes config: ${err instanceof Error ? err.message : String(err)}`);
        return {
            entries: [],
 ...
 */
export function resolveScopes(sessionKey) {
    if (!sessionKey) {
        return ["public"];
    }
